...
namespace Moonfish.Tags
{
    ...
    public partial class GlobalGeometryBlockInfoStruct
    {
        ...
        public virtual GlobalGeometryBlockResourceBlock[] ReadResources(BinaryReader binaryReader)
        {
            var elementSize = Marshal.SizeOf(typeof(GlobalGeometryBlockResourceBlock));
            var blamPointer = binaryReader.ReadBlamPointer(elementSize);
            var resources = new GlobalGeometryBlockResourceBlock[blamPointer.Count];
            using (binaryReader.BaseStream.Pin())
            {
                for (int i = 0; i < blamPointer.Count; ++i)
                {
                    binaryReader.BaseStream.Position = blamPointer[i];
                    resources[i] = new GlobalGeometryBlockResourceBlock(binaryReader);
                }
            }
            return resources;
        }
